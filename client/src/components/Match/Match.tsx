...
export default function Match({setCurrentPage}: {setCurrentPage: Dispatch<string>}) {
    ...
    function getMatches() {
        axios.get('http://127.0.0.1:8000/api/swipesMatchList/')
        .then(function (response) {
            setMatches(response.data.swipesMatch);
        })
    ...
    return (
        <div style={{ flexGrow: 1, display: 'flex', alignItems: 'center', flexDirection: 'column', backgroundSize: 'cover', backgroundPosition: 'center', backgroundImage: 'url(https://i.pinimg.com/originals/81/29/c4/8129c47eea4ca2923d834a0daf316d72.jpg)', backdropFilter: 'blur(15px)' }}>
            <Card sx={{background: '#121212', paddingBottom: theme.spacing(1), marginTop: theme.spacing(1)}}>
                {
                matches === null ? (
                    [0].map(i => {
                    ...
                    matches.map((match, ind) => {
                        ...
                        return (
                            <Card sx={{ display: 'flex', width: '-webkit-fill-available', marginTop: theme.spacing(1), marginInline: theme.spacing(1), alignItems: 'center', maxWidth: maxCardWidth }}>
                                <CardMedia
                                component="img"
                                ...
                                image={`http://127.0.0.1:8000/media/${match.image}`}
                                ...
                                <CardContent style={{maxWidth: window.innerWidth > maxCardWidth ? maxCardWidth-profileImageWidth-8 : `calc(100vw - ${profileImageWidth}px - ${cardContentPaddings} - ${cardContentPaddings})`, padding: cardContentPaddings}}>
