...
export default function Profile({setCurrentPage, openModal, setOpenModal}: {setCurrentPage: Dispatch<string>, openModal: string | null, setOpenModal: Dispatch<string | null>}) {
    ...
    async function fetchProfile() {
        ...
        await axios.get(`${API_URL}/profileData/`, {
            headers: {
    ...
    function useFormInput(label: string, helperText: string, pattern: string, required?: boolean) {
        ...
        function onChange(ev: React.ChangeEvent<HTMLInputElement>) {
            ...
            setError((!ev.target.validity.valid || (ev.target.value === '' && required)) && validate);
        }

        return {
            label: label,
            value: value,
            error: (error && validate) || (required ? (value === '' && validate) : false),
            onChange: onChange,
            helperText: validate && (error || (required ? (value === '' && validate) : false)) && helperText,
    ...
    function fetchSaveData(fetchingArrName: string, endPointName: string, setState: Dispatch<React.SetStateAction<[] | Hobby[]>>) {
        axios.get(`${API_URL}/${endPointName}/`)
        .then(response => {
            if (response.status != 200) return
            setState(response.data[fetchingArrName])
            if (fetchingArrName == 'hobbies') {
                setPreselectedHobbies(response.data[fetchingArrName].filter((hobby: Hobby) => selectedHobbiesIds.includes(hobby.id)))
    ...
    const emailInputProps = useFormInput('Эл. почта', 'Введите действительный адрес электронной почты по типу example@mail.com', "^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$", true);
    const passwordInputProps = useFormInput('Пароль', 'Пароль должен содержать от 8 до 50 символов, среди которых есть только латинские символы (минимум одна заглавная и строчная буквы), минимум одна цифра (0-9) и минимум один символ из набора: !@#$%^&*_=+-', "^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\\$%\\^&\\*_=\\+\\-]).{8,50}$", true);
    ...
    useEffect(() => {
        if (profileCard && allHobbies) {
            ...
            setPreselectedHobbies(allHobbies.filter((hobby: Hobby) => selectedIds.includes(hobby.id)))
            const buildingOption = buildingParams.data.find((option: Hobby) => option.name === profileCard.building_name);
            const departmentOption = departmentParams.data.find((option: Hobby) => option.name === profileCard.department_name);
            const courseOption = courseParams.data.find((option: Hobby) => option.name === profileCard.course_name);
            ...
            console.log("???",allHobbies.filter((hobby: Hobby) => selectedIds.includes(hobby.id)))
        }
    }, [allHobbies])

    let theme = useTheme();

    const validationErrorMessage = 'Ого, что-то пошло не так с отправкой формы! Проверьте, все ли поля заполнены правильно, и попробуйте снова.';
    ...
    const handleSubmitForm = (ev: React.FormEvent<HTMLFormElement>) => {
        ...
        if (vkInputProps.value && !vkInputProps.value.startsWith('https://vk.com/')) {
        ...
        <div className="wallpaperBackground" style={{ flexGrow: 1, display: 'grid'}}>
            ...
            >
                <Card style={{ outline: 'none', padding: 0 }} sx={modalStyle}>
                    <form onSubmit={handleSubmitForm} noValidate encType="multipart/form-data">
                    {validationError && <Alert severity="error" style={{width: 'inherit', position: 'sticky', top: 0, zIndex: 300}}>{validationErrorMessage}</Alert>}
                    <CardContent>
                                ...
                                renderInput={(params) => {
                                    ...
                                    if (!selectedHobbiesIds) {
                                        if (params.InputProps.startAdornment instanceof Array) {
                                            setSelectedHobbiesIds(hobbiesToIds(params.InputProps.startAdornment.map((el: any) => el.props['label'])));
                                          } else if (params.InputProps.startAdornment instanceof Object && 'props' in params.InputProps.startAdornment) {
                                            setSelectedHobbiesIds(hobbiesToIds([params.InputProps.startAdornment.props['label']]));
                                          }
                                    }
                        ...
                        <Typography gutterBottom variant="subtitle1" mt={2} color={validate && (tgInputProps.value === '' && vkInputProps.value === '') || tgInputProps.error || vkInputProps.error ? "error" : ""}>
                    ...
                    <CardActions style={{display: 'flex', justifyContent: 'space-between'}}>
                        ...
                        <Button color="error" type="submit" formNoValidate>Сохранить</Button>
                    </CardActions>
                    </form>
                </Card>
            </Modal>
