...
export default function SignUp({ setCurrentPage, loggedIn, setLoggedIn }) {
  ...
  const hobbiesToIds = (hobbies) => {
    return hobbies?.map(
      (hobby) => allHobbies.filter((h) => h.name === hobby)[0].id
    );
  };
  ...
  function useFormInput(label, helperText, pattern, required) {
    ...
    return {
      ...
      onChange: onChange,
      helperText:
        (error || (required ? value === "" && validate : false)) && helperText,
  ...
  const validationErrorMessage =
    "Ого, что-то пошло не так! Похоже, форма запуталась. Проверьте, все ли поля заполнены правильно, и попробуйте снова.";
  ...
  return (
    ...
    >
      {!errorMessage && !isLoading && (
        <form
          style={{ width: "clamp(300px, 90vw, 500px)" }}
        ...
        >
          {validationError && (
            <Alert
              severity="error"
              style={{ width: "inherit", position: "absolute", zIndex: 300 }}
            >
          ...
          >
            <CardContent>
              ...
              <Box sx={{ display: "grid", flexDirection: "column", gap: 1 }}>
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => {
                    ...
                    selectedHobbiesIds = hobbiesToIds(
              ...
              <FormGroup style={{ marginTop: theme.spacing(2) }}>
                <FormControlLabel
                  ...
                  onChange={() => setPolicyAgreement((prev) => !prev)}
                  style={
                    !policyAgreement && validate
                      ? { color: theme.palette.error.main }
                      : {}
                  }
                  control={
                ...
                <FormControlLabel
                  ...
                  onChange={() => setAgeConfirmation((prev) => !prev)}
                  style={
                    !ageConfirmation && validate
                      ? { color: theme.palette.error.main }
                      : {}
                  }
                  control={
            ...
            <CardActions>
              <Button size="small" type="submit" formnovalidate>
                Создать аккаунт
              </Button>
            </CardActions>
          </Card>
