...
export default function Swipe({
  ...
  const searchesLove = useSelector(
    (state: RootState) => state.filters.searchesLove
  );
  ...
  const [dragStartAfterThreshold, setDragStartAfterThreshold] =
    useState<number>(NaN);
  ...
  const [acceptColor, rejectColor] = [
    theme.palette.mode === "dark" ? "green" : "#87d687",
    theme.palette.mode === "dark" ? "red" : "#ebae98",
  ];
  ...
  function handleCardDrag(ev: any) {
    ...
    backgroundRef.current!.style["boxShadow"] = (() => {
      if (Math.abs(dragCurrent - dragStart) >= diffForDecision) {
        return `inset ${
          decisionStripeWidth * (dragCurrent > dragStart ? -1 : 1)
        }px 0 100px -100px ${
          dragCurrent > dragStart ? acceptColor : rejectColor
        }`;
      }
      return "";
    })();
  ...
  const [currentBunchOfCards, setCurrentBunchOfCards] = useState<
    cardObj[] | null
  >(null);
  ...
  function fetchNewBunchOfCards(): void | boolean {
    ...
    (isInbox ? axios.get : axios.post)(
      isInbox ? `${API_URL}/incomingProfiles/` : `${API_URL}/userList/`,
      ...conditionalArgs
    )
      ...
      .catch(function (err: any) {
        ...
        if (err.response.status === 404) {
          ...
          setErrorMessage(
            `Ошибка загрузки данных с сервера. Код ошибки: ${err.code}. Пожалуйста, попробуйте зайти на страницу позднее.`
          );
        }
        //return false;
      });
  ...
  useEffect(() => {
    ...
    [cardStyle.marginLeft, cardStyle.transform] = (() => {
      return dragCurrent !== 0
        ? [
            `${(dragCurrent - dragStartAfterThreshold) * cardDragSpeed}px`,
            `rotate(${
              (dragCurrent - dragStartAfterThreshold) * cardRotationSpeed
            }deg)`,
          ]
        : ["0px", "rotate(0deg)"];
    })();
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Box sx={modalStyle}>
            ...
            <Typography id="modal-modal-description" gutterBottom>
              На данной странице можно просматривать анкеты других
              пользователей.
            </Typography>
            <Typography variant="h6" component="h4" gutterBottom>
              Подсказки
            ...
            <Typography id="modal-modal-description" gutterBottom>
              Иногда в тексте встречаются подсказки (
              <Tooltip title="Подсказка">
                <IconButton aria-label="info" size="small">
                  <InfoOutlinedIcon fontSize="inherit" />
                </IconButton>
              </Tooltip>
              ). Чтобы воспользоваться подсказкой, наведите на неё курсор или
              зажмите долгим нажатием.
            </Typography>
            <Typography variant="h6" component="h4" gutterBottom>
              Просмотр анкет
            </Typography>
            <Typography id="modal-modal-description" gutterBottom>
              Чтобы посмотреть больше инфорации о пользователе, прокрутите
              анкету, наведя на неё курсор мыши и покрутив колёсико мыши или
              свайпнув
              ...
              вверх (для устройств с сенсорным экраном).
            </Typography>
            <Typography variant="h6" component="h4" gutterBottom>
              Принятие и отклонение анкет
            </Typography>
            <Typography id="modal-modal-description" gutterBottom>
              Чтобы послать пользователю симпатию, нажмите на галочку внизу
              экрана или сделайте свайп вправо. Чтобы отклонить анкету - крестик
              внизу экрана или свайп влево.
            </Typography>
            <Typography variant="h6" component="h4" gutterBottom>
              Закрытие окон
            </Typography>
            <Typography id="modal-modal-description" gutterBottom>
              Для закрытия модального окна (например, текущего), нажмите в любом
              месте веб-страницы за его пределами.
            </Typography>
          </Box>
        </Modal>
        ...
        {currentBunchOfCards && (
          <ComplaintModal
            targetId={currentBunchOfCards[currentCardId].id}
            openModal={openModal}
        ...
        <Box sx={{ flexGrow: 1 }}>
          ...
          >
            ...
            >
              ...
              >
                {!errorMessage &&
                  ...
                  !noCardsLeft &&
                  cardObjToSwipeCard(
                    currentBunchOfCards[currentCardId],
                    setOpenModal
                  )}
              </div>
              ...
              {!errorMessage &&
                !currentBunchOfCards &&
                !noCardsLeft &&
                !infoMessage && <Alert severity="info">Загрузка...</Alert>}

              {noCardsLeft && (
                <Alert severity="info">
                  Вы свайпнули все карточки, что есть на данный момент по
                  данному фильтру.
                </Alert>
              )}
            </Grid>
            <Grid
              ...
              sx={{ width: "100%" }}
              style={{
                display: currentBunchOfCards && !noCardsLeft ? "block" : "none",
              }}
            >
              {currentBunchOfCards && !noCardsLeft && (
                ...
                >
                  <Tooltip title="Пропустить анкету">
                    <IconButton
                      color="error"
                      sx={{ border: "1px solid", aspectRatio: "1" }}
