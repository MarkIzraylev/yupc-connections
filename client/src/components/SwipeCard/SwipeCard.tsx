    ...
    >
      ...
      >
          ...
          {!isOwnProfile && (
            <Tooltip title="Пожаловаться">
              <IconButton
                ...
                onClick={() => setOpenModal("complaint")}
              >
                <FlagOutlinedIcon />
              </IconButton>
            </Tooltip>
          ...
          {!isOwnProfile && thereIsSocialMedia && (
            <Tooltip title="Отменить метч">
              <IconButton
                size="small"
                sx={{ height: "fit-content" }}
                onClick={() => setOpenModal("remove-match")}
              >
        ...
        <Stack mb={2} spacing={1} direction="row" useFlexGap flexWrap="wrap">
          {mainTags.map((tag) => {
            return <Chip label={tag} variant="outlined" key={tag} />;
          })}
          {intentionTags.map((tag) => {
            return (
              <Chip
                label={tag}
                variant="outlined"
                color={tag.toLowerCase().includes("дру") ? "success" : "error"}
                key={tag}
              />
            );
          })}
        ...
        <Stack spacing={1} direction="row" useFlexGap flexWrap="wrap">
          {hobbiesTags.map((hobby) => {
            return <Chip label={hobby} key={hobby} variant="outlined" />;
          })}
        </Stack>
        {thereIsSocialMedia && (
          <>
